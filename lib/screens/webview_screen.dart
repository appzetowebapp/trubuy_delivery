...
class _WebViewScreenState extends State<WebViewScreen>
  ...
  static const platform = MethodChannel('com.trubuy.delivery1/geolocation');
  ...
  Future<void> _showOverlayWindow() async {
    ...
    await FlutterOverlayWindow.showOverlay(
      enableDrag: true,
      overlayTitle: "Trubuy Delivery Partner Overlay ",
  ...
  Future<Map<String, dynamic>> _encodeImagesForWeb(List<XFile> images) async {
    ...
    return {
      ...
      'files': files,
      if (files.isEmpty) 'error': 'No image selected',
    };
  }

  Future<Map<String, dynamic>> _pickImagesForWeb({
    ...
    try {
      ...
      if (selectedSource == null) {
        if (!mounted) {
          return {'success': false, 'error': 'Screen is no longer available'};
        }
        ...
        if (!status.isGranted) {
          return {
            ...
            'error': status.isPermanentlyDenied
                ? 'Camera permission is disabled in app settings'
      ...
      if (selectedSource == ImageSource.gallery && allowMultiple) {
      ...
      return await _encodeImagesForWeb(images);
    } catch (error, stackTrace) {
      debugPrint('❌ Native image picker failed: $error');
      debugPrint('$stackTrace');
      return {'success': false, 'error': error.toString()};
    }
  }

  Future<void> _injectNativeImagePickerScript(
      InAppWebViewController controller) async {
    const script = r"""
      (function() {
        if (window.__nativeImagePickerInstalled) return;
        window.__nativeImagePickerInstalled = true;

        async function base64ToFile(fileData) {
          ...
          return new File(
            [bytes],
            fileData.name || ('image_' + Date.now() + '.jpg'),
            { type: fileData.mimeType || 'image/jpeg', lastModified: Date.now() }
          );
        }
        ...
        document.addEventListener('click', async function(event) {
          ...
          try {
            var result = await window.flutter_inappwebview.callHandler(
              'pickImagesForWeb',
            ...
            for (var i = 0; i < result.files.length; i++) {
              transfer.items.add(await base64ToFile(result.files[i]));
            }
        ...
        (function() {
          ...
          window.fetch = async function(url, options) {
            ...
            var isLogin = urlString.toLowerCase().includes('login') || 
                          ...
                          urlString.toLowerCase().includes('/delivery/auth/login');
          ...
          XMLHttpRequest.prototype.send = function(data) {
            ...
            if (url && (url.toLowerCase().includes('login') || 
                        ...
                        url.toLowerCase().includes('/delivery/auth/login'))) {
  ...
  callback: (args) async {
    ...
    try {
      ...
      debugPrint('🔐 Captured Login Response: $response');

      // Agar response body ke andar aata hai to body use karo,
      // warna direct response use karo
      ...
      if (body['data'] != null &&
        ...
        if (dataObj['deliveryBoy'] != null &&
            dataObj['deliveryBoy'] is Map<String, dynamic>) {
          final deliveryBoy =
              dataObj['deliveryBoy'] as Map<String, dynamic>;

          phone = deliveryBoy['phone']?.toString();
        }
      }

      if (phone != null && phone.isNotEmpty) {
        debugPrint('📱 Phone Found: $phone');
        ...
        debugPrint('✅ Phone Saved: $cleanedPhone');
      } else {
                  ...
                  children: [
                    InAppWebView(
                      ...
                      onWebViewCreated: (controller) async {
                        ...
                        // Native picker used by image file inputs in the web app.
                        controller.addJavaScriptHandler(
                          handlerName: 'pickImagesForWeb',
                          callback: (args) async {
                            final options = args.isNotEmpty && args.first is Map
                                ? Map<dynamic, dynamic>.from(args.first as Map)
                                : <dynamic, dynamic>{};
                        ...
                        // Backward-compatible bridge for pages that explicitly
                        // request the camera.
                        controller.addJavaScriptHandler(
                          handlerName: 'openCamera',
                          callback: (args) async {
                            debugPrint('🟢 openCamera JS handler triggered');
                            final result = await _pickImagesForWeb(
                              allowMultiple: false,
                            ...
                            if (result['success'] == true &&
                                ...
                                files.isNotEmpty &&
                                files.first is Map) {
                              final file = Map<dynamic, dynamic>.from(
                                  files.first as Map);
                              return {
                                ...result,
                                'base64': file['base64'],
                      ...
                      onLoadStop: (controller, url) async {
                        ...
                        await _injectNativeImagePickerScript(controller);
                      },
